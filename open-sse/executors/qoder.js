...
async function buildQoderRequestBody({ model, body, credentials, log, proxyOptions, signal }) {
  ...
  if (!modelConfig) {
    ...
    if (!retried) {
      console.log("QODER FALLBACK: trying fallback for", qoderKey, "| refreshed?", !!refreshed, "| rawConfigs keys:", refreshed ? [...refreshed.rawConfigs.keys()].join(",") : "null");
      // Fallback: clone config from a known model when the target model
      // exists upstream but isn't in this account's catalog yet.
      // Verified safe: upstream accepts config from a sibling model.
      const FALLBACK_CONFIG_MAP = {
        cmodel: "qmodel_38max",
      };
